...
The credential helpers read AWS access keys from 1Password with `op` and emit the JSON shape expected by AWS CLI `credential_process`. They require `op` and `jq` on `PATH`.
